...
export default function ContactForm() {
  const { data, setData, submit, processing, errors, wasSuccessful, reset } = useForm({
    name: '',
    city: '',
    email: '',
    phone: '',
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative mx-auto max-w-2xl px-6 pt-12 pb-10 text-center">
            ...
            <div className="mt-5 space-y-3 text-left text-sm leading-relaxed text-gray-600 sm:text-base">
              ...
              <p>Vous souhaitez nous faire remonter une problématique&nbsp;?</p>
              <p>Vous souhaitez proposer une collaboration ?</p>
              <p className="mt-2 font-semibold text-sna-teal-dark">Le Syndicat National des Aidants est à votre écoute.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-2xl px-6 pb-20">
          {wasSuccessful ? (
            <div className="space-y-4 rounded-3xl border border-teal-200 bg-teal-50 p-12 text-center shadow-lg">
              <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-teal-100 text-4xl">✅</div>
              <h2 className="text-2xl font-bold text-teal-800">Message envoyé !</h2>
              <p className="text-teal-700">Votre message a bien été transmis. Merci pour votre prise de contact.</p>
              <Link
                href="/"
            ...
            <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-gray-100 bg-white p-8 shadow-sm">
              <h2 className="text-sm font-bold tracking-wide text-sna-teal-dark uppercase">Objet de votre demande</h2>
              <div>
              ...
              <h2 className="text-sm font-bold tracking-wide text-sna-teal-dark uppercase">Vos coordonnées</h2>
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                  <label className={labelCls}>Téléphone (facultatif)</label>
                  <input
                    type="tel"
                    value={data.phone}
              ...
              <h2 className="text-sm font-bold tracking-wide text-sna-teal-dark uppercase">Vous êtes</h2>
              <div className="flex flex-wrap gap-4">
                {profiles.map((option) => (
                  <label key={option.value} className="flex cursor-pointer items-center gap-2 text-sm text-gray-600">
                    <input
                      type="radio"
                      name="profile"
                      checked={data.profile === option.value}
                      onChange={() => setData('profile', option.value)}
              ...
              {errors.profile && <p className="mt-1 text-xs text-red-500">{errors.profile}</p>}

              <h2 className="text-sm font-bold tracking-wide text-sna-teal-dark uppercase">Souhaitez-vous être recontacté(e) ?</h2>
              <div className="flex flex-wrap gap-4">
                {contactPrefs.map((option) => (
                  <label key={option.value} className="flex cursor-pointer items-center gap-2 text-sm text-gray-600">
                    <input
                      ...
                      checked={data.contact_preference === option.value}
                      onChange={() => setData('contact_preference', option.value)}
                      className="accent-sna-teal"
                    />
                    {option.label}
                  </label>
              ...
              {errors.contact_preference && <p className="mt-1 text-xs text-red-500">{errors.contact_preference}</p>}

              <div className="space-y-2 border-t border-gray-50 pt-4">
                <h2 className="text-sm font-bold tracking-wide text-sna-teal-dark uppercase">Mention légale</h2>
                <div className="text-xs text-gray-600 space-y-1">
                  <p>Les informations recueillies sont utilisées exclusivement dans le cadre des activités du Syndicat National des Aidants.</p>
                  <p>Elles ne sont ni cédées ni vendues.</p>
                  <p>Vous pouvez exercer votre droit d’accès, de rectification ou de suppression à tout moment.</p>
                  <p>Nous ne pouvons pas toujours répondre immédiatement aux situations urgentes.<br />En cas d’urgence sociale ou médicale, contactez les services compétents.</p>
                </div>
              </div>
              ...
              >
                {processing ? 'Envoi en cours…' : 'Envoyer mon message'}
              </button>
            </form>
          )}
        </div>
